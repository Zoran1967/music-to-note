...
class RecordingsScreen(MDScreen):
    ...
    def _save_to_sheet_music(self, notes, source_path):
        ...
        try:
            ...
            if not hasattr(app, "sheet_storage") or app.sheet_storage is None:
                print("Sheet storage nije inicijalizovan")
                return

            # Podrazumevano ime na osnovu izvornog fajla + rednog broja
            base = os.path.splitext(os.path.basename(source_path))[0]
            existing = app.sheet_storage.get_all_entries()
            default_name = "{} ({})".format(base, len(existing) + 1)

            entry = app.sheet_storage.add_entry(default_name, notes)
            print("Sacuvan zapis:", entry["name"])

            # Navigiraj na SheetMusicScreen (lista)
            app.root.current = "sheet_music"
        except Exception as e:
            print("Greska pri cuvanju u notne zapise:", e)

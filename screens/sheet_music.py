...
from config import icon, COLORS, hex_to_rgba


NOTE_LETTERS = ["C", "D", "E", "F", "G", "A", "B"]
STAFF_LINE_GAP = dp(9)
STAFF_TOP_MARGIN = dp(20)
STAFF_BOTTOM_MARGIN = dp(20)
STAFF_LINES = 5
...
class StaffCanvas(Widget):
    ...
    def _draw_notes(self):
        ...
        min_dur = min(n["end"] - n["start"] for n in self.notes) or 1.0
        max_dur = max(n["end"] - n["start"] for n in self.notes) or 1.0
        ...
        for n in self.notes:
            ...
            # Ispis imena note ispod
            Color(*hex_to_rgba(COLORS["text_dim"], 0.8))
            Line(points=[], width=0)
            self._draw_text(x, y - dp(30), n["note"])

            x += note_width

    def _draw_text(self, x, y, text):
        """Pojednostavljeni prikaz imena note."""
        # U Kivy Canvas ne možemo direktno tekst bez Label widget-a.
        # Rešićemo ovo dodavanjem Label widget-a kao child.
        label = MDLabel(
            text=text,
            font_size=10,
...
class SheetMusicScreen(MDScreen):
    ...
    def __init__(self, **kwargs):
        ...
        back_btn.bind(on_touch_down=self._go_back)
        ...
        self.staff_canvas = StaffCanvas(
            size_hint=(None, 1),
            width=dp(500),  # minimalna širina, biće proširena
        )
    ...
    def set_notes(self, notes):
        ...
        self._notes = notes
        self.staff_canvas.notes = notes

        if not notes:
            self.status_label.text = "Nema prepoznatih nota"
        else:
            self.status_label.text = "{} nota".format(len(notes))
            # Prilagodi širinu Canvas-a broju nota
            total_width = dp(80) + len(notes) * dp(50)
